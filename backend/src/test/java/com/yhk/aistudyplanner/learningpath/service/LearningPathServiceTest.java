...
class LearningPathServiceTest {
    ...
    LearningPathService service; LocalDate today=LocalDate.of(2026,8,23);
    ...
    @Test void confirmAllowsTenDayWindow(){
        when(subjectService.requireOwned(2L,1L)).thenReturn(subject());
        when(pathMapper.insert(any(StudyPath.class))).thenAnswer(invocation->{((StudyPath)invocation.getArgument(0)).setId(7L);return 1;});
        when(itemMapper.selectRows(7L,1L)).thenReturn(List.of());
        assertEquals(LocalDate.of(2026,9,1),service.confirm(confirm()).targetDate());
    }
    @Test void confirmRejectsElevenDayWindow(){
        BusinessException error=assertThrows(BusinessException.class,()->service.confirm(confirm(LocalDate.of(2026,9,2))));
        ...
        verifyNoInteractions(subjectService,pathMapper,itemMapper);
    }
    @Test void confirmWithoutTargetDateKeepsOpenEndedPathSemantics(){
        when(subjectService.requireOwned(2L,1L)).thenReturn(subject());
        when(pathMapper.insert(any(StudyPath.class))).thenAnswer(invocation->{((StudyPath)invocation.getArgument(0)).setId(7L);return 1;});
        when(itemMapper.selectRows(7L,1L)).thenReturn(List.of());
        assertNull(service.confirm(confirm(null)).targetDate());
    }
    @Test void confirmReturnsDetailWithJsonMinutesAndJoinedTaskFields(){when(subjectService.requireOwned(2L,1L)).thenReturn(subject());when(pathMapper.insert(any(StudyPath.class))).thenAnswer(i->{((StudyPath)i.getArgument(0)).setId(7L);return 1;});var mapped=new LearningPathItemMapper.ItemRow(3L,1,1,"集合基础",null,"HashMap","理解put流程与扩容机制","[\"阅读源码\",\"运行示例\"]","[\"能解释put流程\"]",90,3,"面向对象","面试高频知识点","PENDING",11L,"复习HashMap","TODO");when(itemMapper.selectRows(7L,1L)).thenReturn(List.of(mapped));var result=service.confirm(confirm());assertEquals(1,result.items().size());var item=result.items().get(0);assertEquals(List.of("阅读源码","运行示例"),item.learningMethod());assertEquals(List.of("能解释put流程"),item.completionCriteria());assertEquals(90,item.estimatedMinutes());assertEquals("复习HashMap",item.taskTitle());assertEquals("TODO",item.taskStatus().name());}
    @Test void confirmPersistsUsersFinalEditedDraftWithoutLosingFields(){when(subjectService.requireOwned(2L,1L)).thenReturn(subject());when(pathMapper.insert(any(StudyPath.class))).thenAnswer(i->{((StudyPath)i.getArgument(0)).setId(7L);return 1;});when(itemMapper.selectRows(7L,1L)).thenReturn(List.of());var edited=new LearningPathConfirmRequest(2L,"用户修改后的Java路线","修改后的摘要","十天复习Java",null,"基础一般","通过Java后端面试",null,null,today,today.plusDays(9),120,List.of(new LearningPathConfirmRequest.Item(1,1,"Java集合",null,"HashMap扩容机制","理解JDK8 HashMap结构、put流程和扩容条件",List.of("编写put与get示例","分析一次扩容过程"),List.of("能解释put流程和扩容条件","能说明线程不安全原因"),90,4,"面向对象和集合体系","面试高频知识点")));service.confirm(edited);var captor=ArgumentCaptor.forClass(StudyPathItem.class);verify(itemMapper).insert(captor.capture());var saved=captor.getValue();assertEquals("HashMap扩容机制",saved.getTopic());assertEquals("理解JDK8 HashMap结构、put流程和扩容条件",saved.getLearningObjective());assertEquals("[\"编写put与get示例\",\"分析一次扩容过程\"]",saved.getLearningMethod());assertEquals("[\"能解释put流程和扩容条件\",\"能说明线程不安全原因\"]",saved.getCompletionCriteria());assertEquals(90,saved.getEstimatedMinutes());assertEquals(4,saved.getSuggestedDay());assertEquals("面向对象和集合体系",saved.getPrerequisiteText());assertEquals("面试高频知识点",saved.getReason());}
    @Test void confirmRejectsFutureTopicAsPrerequisite(){var base=confirm().items().get(0);var items=List.of(new LearningPathConfirmRequest.Item(1,1,"基础",null,"面向对象",base.learningObjective(),base.learningMethod(),base.completionCriteria(),60,1,"HashMap",null),new LearningPathConfirmRequest.Item(2,2,"集合",null,"HashMap","理解JDK8 HashMap结构和put与扩容流程",List.of("编写HashMap示例并分析put流程"),List.of("能解释HashMap扩容条件"),60,2,"面向对象",null));var request=new LearningPathConfirmRequest(2L,"Java路线","摘要","复习Java",null,"一般","面试",null,null,today,today.plusDays(9),120,items);when(subjectService.requireOwned(2L,1L)).thenReturn(subject());assertThrows(BusinessException.class,()->service.confirm(request));verify(pathMapper,never()).insert(any(StudyPath.class));}
    ...
    private LearningPathConfirmRequest confirm(){return confirm(LocalDate.of(2026,9,1));}
    private LearningPathConfirmRequest confirm(LocalDate targetDate){return new LearningPathConfirmRequest(2L,"Java路线","摘要","十天复习Java",null,"一般","面试",null,null,today,targetDate,120,List.of(new LearningPathConfirmRequest.Item(1,1,"基础",null,"面向对象","理解封装继承多态及对象模型的核心概念",List.of("阅读面向对象核心概念","编写继承示例代码"),List.of("能解释封装继承和多态的区别"),60,1,null,null)));}
