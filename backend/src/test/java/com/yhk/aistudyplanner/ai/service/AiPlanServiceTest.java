...
class AiPlanServiceTest {
  ...
  @Mock ObjectProvider<PlanningGateway> provider;
  @Mock PlanningGateway gateway;
  @Mock AuthSessionService session;
  @Mock AiPlanningContextService contexts;
  @Mock AiPlanPromptService prompts;
  @Mock RuleBasedPlanGenerator rules;
  AiPlanningProperties properties;
  AiPlanService service;
  ...
  void setUp() {
    ...
    lenient().when(prompts.user(any(), any())).thenReturn("user");
    service =
        new AiPlanService(
            properties,
            provider,
            ...
            contexts,
            prompts,
            new AiPlanResponseParser(new ObjectMapper()),
            new AiPlanResponseValidator(),
            rules,
            Clock.fixed(
  ...
  @Test
  void missingApiKeyFallsBackWithoutCallingProvider() {
  ...
  @Test
  void validJsonCreatesDeterministicDraft() {
    available();
    when(contexts.build(1, DATE)).thenReturn(context());
    when(gateway.generate(anyString(), anyString()))
        .thenReturn(
            "{\"summary\":\"ok\",\"items\":[{\"taskId\":10,\"plannedMinutes\":60,\"reason\":\"due\"},{\"taskId\":11,\"plannedMinutes\":45,\"reason\":\"progress\"}]}");
    var r = service.generate(request);
    assertEquals("AI", r.generatorType());
    assertFalse(r.fallbackUsed());
    assertEquals(105, r.draft().plannedMinutes());
    assertEquals(DATE.atTime(9, 0), r.draft().items().get(0).startAt());
    assertEquals(r.draft().items().get(0).endAt(), r.draft().items().get(1).startAt());
  }

  @Test
  void emptyAndInvalidJsonFallback() {
    assertFallbackFor(null);
    assertFallbackFor("bad json");
  }

  @Test
  void inventedAndDuplicateTaskFallback() {
    assertFallbackFor(json("99,30,reason"));
    assertFallbackFor(
        "{\"summary\":\"x\",\"items\":[{\"taskId\":10,\"plannedMinutes\":30,\"reason\":\"x\"},{\"taskId\":10,\"plannedMinutes\":30,\"reason\":\"x\"}]}");
  }
  ...
  @Test
  void timeoutRateLimitAndUnavailableFallbackWithoutLeakingError() {
    for (ErrorCode code :
        List.of(
      ...
      available();
      when(contexts.build(1, DATE)).thenReturn(context());
      when(gateway.generate(anyString(), anyString())).thenThrow(new AiProviderException(code));
      var result = service.generate(request);
      assertTrue(result.fallbackUsed());
      assertFalse(result.fallbackReason().contains("test-key"));
      reset(gateway);
    }
  }

  private void assertFallbackFor(String content) {
    available();
    when(contexts.build(1, DATE)).thenReturn(context());
  ...
  private String json(String csv) {
    String[] p = csv.split(",", 3);
    return "{\"summary\":\"x\",\"items\":[{\"taskId\":"
        + p[0]
        + ",\"plannedMinutes\":"
        + p[1]
        + ",\"reason\":\""
  ...
  private AiPlanningContext.TaskContext task(long id, int minutes) {
