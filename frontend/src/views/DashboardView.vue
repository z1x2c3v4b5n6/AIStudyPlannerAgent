...
<template>
  <section class="page-section">
    ...
    <div class="stat-grid">
      ...
      <el-card v-loading="todayLoading" shadow="never" class="stat-card"><span>今日任务</span><strong>{{ todayFailed ? '—' : today.tasks.length }}</strong><small v-if="todayFailed">加载失败</small></el-card>
      <el-card v-loading="todayLoading" shadow="never" class="stat-card"><span>今日预计学习</span><strong class="minutes-value">{{ todayFailed ? '—' : minutesLabel(today.totalEstimatedMinutes) }}</strong><small v-if="todayFailed">加载失败</small></el-card>
    </div>
    <div class="dashboard-grid">
      <el-card shadow="never" class="panel-card dashboard-panel"><template #header><div class="card-header"><strong>今日任务</strong><el-button text type="primary" @click="$router.push('/tasks')">查看全部</el-button></div></template><div v-loading="todayLoading"><el-alert v-if="todayFailed" type="error" title="今日任务加载失败" :closable="false" show-icon><el-button link type="primary" @click="loadToday">重新加载</el-button></el-alert><el-empty v-else-if="!today.tasks.length" description="今天还没有待完成任务" :image-size="80" /><div v-else class="task-stack"><article v-for="task in today.tasks" :key="task.id" class="task-row"><div><strong>{{ task.title }}</strong><p>{{ subjectNames.get(task.subjectId) || '未知科目' }} · {{ minutesLabel(task.estimatedMinutes) }}</p></div><div class="task-row-meta"><el-tag :type="priorityMap[task.priority].type">{{ priorityMap[task.priority].label }}</el-tag><el-tag :type="taskStatusMap[task.status].type">{{ taskStatusMap[task.status].label }}</el-tag><el-button v-if="task.status === 'TODO'" size="small" :loading="changingTaskId === task.id" @click="changeStatus(task, 'IN_PROGRESS')">开始</el-button><el-button type="primary" size="small" :loading="changingTaskId === task.id" @click="changeStatus(task, 'COMPLETED')">完成</el-button></div></article></div></div></el-card>
