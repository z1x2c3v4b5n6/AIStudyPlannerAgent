...
import { shanghaiDate } from '../../utils/businessTime'

const examples=['我Java基础一般，希望三周系统复习Java基础，为Java后端面试做准备，每天2小时。','我想用四周补齐Python基础，每天学习90分钟，节奏不要太紧。','为数据库面试设计两周复习路线，重点是MySQL索引和SQL。']
...
async function generate(){if(!form.requirement.trim())return ElMessage.warning('请先描述长期学习目标');if(form.startDate<shanghaiDate())return ElMessage.warning('开始日期不能早于今天');if(form.targetDate&&form.startDate>form.targetDate)return ElMessage.warning('目标日期不能早于开始日期');if(form.dailyMinutes<15||form.dailyMinutes>720)return ElMessage.warning('每日学习时间必须在15至720分钟之间');generating.value=true;try{draft.value=(await learningPathApi.generate({...form,targetDate:form.targetDate||null})).data.data}catch{draft.value=null}finally{generating.value=false}}
...
<template>
  <section class="page-section learning-path-page">
    ...
    <el-card class="path-generate-card" shadow="never">
      ...
      <div class="path-prompt-shell"><el-input v-model="form.requirement" type="textarea" :rows="4" maxlength="2000" show-word-limit placeholder="例如：我Java基础一般，希望三周系统复习Java基础，为Java后端面试做准备，每天2小时。" /></div>
      ...
      <el-collapse class="path-settings"><el-collapse-item title="附加设置">
        <el-form label-position="top" class="path-settings-grid">
          ...
          <el-form-item label="目标日期"><el-date-picker v-model="form.targetDate" type="date" value-format="YYYY-MM-DD" clearable /></el-form-item>
      ...
      <div class="path-generate-footer"><span><i></i> 生成内容仅作为草案，确认后才会保存</span><el-button type="primary" class="primary-action path-generate-button" :disabled="!form.requirement.trim()||generating" :loading="generating" @click="generate">{{ generating?'正在为你设计学习路线...':'✦ AI生成学习路线' }}</el-button></div>
    </el-card>

    <el-card v-if="draft" shadow="never" class="path-draft-card">
      <div class="path-draft-heading"><div><div class="path-section-kicker">AI GENERATED ROADMAP</div><el-input v-model="draft.title" class="path-title-input" /><div class="path-draft-metrics"><span><b>{{ draft.stages.length }}</b> 阶段</span><span><b>{{ draftItems.length }}</b> 知识点</span><span><b>{{ draftItems.reduce((n,i)=>n+i.estimatedMinutes,0) }}</b> 分钟</span></div></div><el-button type="primary" class="primary-action" :loading="confirming" @click="confirm">确认学习路径</el-button></div>
      <div class="path-stage-track"><section v-for="(stage,si) in draft.stages" :key="stage.stageNo" class="path-stage"><div class="path-stage-node"><span>{{ stage.stageNo }}</span></div><div class="path-stage-title"><span>PHASE {{ String(stage.stageNo).padStart(2,'0') }}</span><el-input v-model="stage.title" /></div><el-input v-model="stage.description" type="textarea" :rows="2" placeholder="阶段说明" />
        <article v-for="(item,ii) in stage.items" :key="item.sequenceNo" class="path-item-card">
          ...
          <el-input v-model="item.topic" class="path-topic-input" /><label>学习目标</label><el-input v-model="item.learningObjective" type="textarea" :rows="2" /><div class="path-item-columns"><div><label>建议学习方式（每行一项）</label><el-input :model-value="item.learningMethod.join('\n')" type="textarea" :rows="4" @update:model-value="(value:string)=>updateMethods(item,value)" /></div><div><label>完成标准（每行一项）</label><el-input :model-value="item.completionCriteria.join('\n')" type="textarea" :rows="4" @update:model-value="(value:string)=>updateCriteria(item,value)" /></div></div><div class="path-item-schedule"><el-input-number v-model="item.estimatedMinutes" :min="15" :max="1440" /><span class="muted"> 分钟</span><el-input-number v-model="item.suggestedDay" :min="1" /><span class="muted">建议 Day</span></div><label>前置知识</label><el-input v-model="item.prerequisite" clearable /><label>规划理由</label><el-input v-model="item.reason" type="textarea" :rows="2" />
        </article>
      </section></div>
    </el-card>
      </el-tab-pane>
    ...
    <el-drawer v-model="detailVisible" title="学习路径详情" size="min(720px, 94vw)">
    ...
    <el-drawer v-model="itemDetailVisible" title="学习节点详情" size="min(560px, 94vw)" append-to-body>
      <template v-if="selectedItem">
        ...
        <section v-if="selectedItem.reason" class="path-node-detail-section"><h3>规划理由</h3><p>{{ selectedItem.reason }}</p></section>
