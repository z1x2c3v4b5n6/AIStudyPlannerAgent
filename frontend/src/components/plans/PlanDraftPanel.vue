...
const generateButtonText = computed(() =>
  generationMode.value === 'AI' ? 'AI生成计划' : '规则生成计划',
)
...
async function generate() {
  if (!form.planDate) return ElMessage.warning('请选择计划日期')
  if (!form.startTime) return ElMessage.warning('请选择开始时间')
  if (form.planDate < shanghaiDate()) return ElMessage.warning('计划日期不能早于今天')
  if (form.availableMinutes < 1 || form.availableMinutes > 720) {
    return ElMessage.warning('可用时长必须为1至720分钟')
  }
  const end = addWallMinutes(`${form.planDate}T${form.startTime}`, form.availableMinutes)
  if (end.slice(0, 10) !== form.planDate) {
    return ElMessage.warning('计划时间不能跨越自然日')
  }

  const requestedMode = generationMode.value
  const payload = { ...form, requirement: nullableText(form.requirement) }
...
<template>
  <div class="plan-builder">
    <el-card shadow="never" class="panel-card">
      <template #header><strong>生成计划</strong></template>
      <el-form label-position="top">
        <el-form-item label="生成模式">
          <el-radio-group v-model="generationMode" :disabled="generating" class="plan-mode-switch">
            <el-radio-button value="AI">AI智能规划</el-radio-button>
        ...
        <el-form-item label="可用学习时长">
          <el-radio-group v-model="form.availableMinutes">
            <el-radio-button v-for="m in [60, 90, 120, 180]" :key="m" :value="m">
              {{ m }}分钟
            </el-radio-button>
          </el-radio-group>
          <el-input-number v-model="form.availableMinutes" :min="1" :max="720" class="plan-custom-minutes" />
        </el-form-item>
        <el-form-item label="补充要求">
          <el-input v-model="form.requirement" type="textarea" :rows="3" maxlength="1000" show-word-limit />
        </el-form-item>
        <p v-if="generating && generationMode === 'AI'" class="plan-generating-hint">
          AI正在分析任务和近期学习情况，请稍候
        </p>
        <el-button type="primary" :loading="generating" :disabled="generating" @click="generate">
          {{ generateButtonText }}
        </el-button>
      </el-form>
    </el-card>
    ...
    <el-card shadow="never" class="panel-card">
      ...
      <template v-else>
        <el-alert
          ...
          :title="generationMeta.fallbackReason || 'AI暂时不可用，已使用规则生成可用草案'"
          type="warning"
          :closable="false"
          show-icon
        ...
        <div class="draft-summary">
          ...
          <small>
            可用 {{ minutesLabel(draft.availableMinutes) }} · 已排 {{ minutesLabel(draft.plannedMinutes) }} ·
            剩余 {{ minutesLabel(remaining) }}
          </small>
        </div>
        ...
        <div v-else class="draft-items">
          <article v-for="(item, index) in draft.items" :key="item.taskId" class="draft-item">
            ...
            <div class="draft-item-content">
              ...
              <p>
                {{ item.subjectName }} · {{ item.startAt.slice(11, 16) }}—{{ item.endAt.slice(11, 16) }} ·
                {{ item.reason }}
              </p>
            </div>
            <el-input-number
